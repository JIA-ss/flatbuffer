 ...
 private:
  ...
  std::string GenStructMemberValueTS(const StructDef &struct_def,
    ...
    for (auto it = struct_def.fields.vec.begin();
      ...
      const auto curr_member_accessor = prefix + "." + field.name;
      if (IsStruct(field.value.type)) {
    ...
    for (auto it = struct_def.fields.vec.begin();
      ...
      if (IsScalar(field.value.type.base_type) || is_string) {
        ...
        if (field.value.type.base_type != BASE_TYPE_STRING) {
          field_offset_val = "this." + field.name;
        } else {
          field_offset_decl = GenNullCheckConditional(
              "this." + field.name,
              "builder.createString(this." + field.name + "!)", "0");
        }
      }
        ...
        switch (field.value.type.base_type) {
          case BASE_TYPE_STRUCT: {
            ...
            auto packing = GenNullCheckConditional(
                "this." + field.name, "this." + field.name + "!.pack(builder)",
                "0");
          ...
          case BASE_TYPE_VECTOR: {
            ...
            switch (vectortype.base_type) {
              case BASE_TYPE_STRUCT: {
                ...
                if (sd.fixed) {
                  field_offset_decl =
                      "builder.createStructOffsetList(this." + field.name +
              ...
              default: {
                ...
                field_offset_decl = AddImport(imports, struct_def, struct_def) +
                                    ...
                                    "Vector(builder, this." + field.name + ")";
          ...
          case BASE_TYPE_UNION: {
            ...
            field_val = GenUnionValTS(imports, struct_def, field_name,
                                      field.value.type);
            field_offset_decl =
                "builder.createObjectOffset(this." + field.name + ")";
            break;
          }
      ...
      unpack_to_func += "  _o." + field.name + " = " + field_val + ";";

      constructor_func += "  public " + field.name + ": " + field_type + " = " +
                          field_default_val;
